...
def ch_corr_Rayleigh_by_cov(x, AWGN_std, device, Cov_h = None):
    x = x.to(device)
    bs = x.size(0)
    n = x.size(1)
    assert Cov_h is not None, 'Channel covariance matrix, Cov_h, is not given.'
    assert Cov_h.shape == (n, n)
    assert x.dtype == torch.cfloat, 'The channel input should be defined to have the data type of torch.cfloat. '
